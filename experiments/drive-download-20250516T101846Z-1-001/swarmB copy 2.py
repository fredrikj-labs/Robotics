...
pygame.display.set_caption("Swarm: Honest Distributed Map (Bidirectional)")
...
def merge_pair(botA, botB):
    if robots_see_each_other(botA, botB) or robots_see_each_other(botB, botA):
        # Merge explored maps
        combined_map = np.logical_or(botA.local_explored, botB.local_explored)
        botA.local_explored = combined_map.copy()
        botB.local_explored = combined_map.copy()
        # Merge goal info
        if botA.goal_grid and not botB.goal_grid:
            botB.goal_grid = botA.goal_grid
        elif botB.goal_grid and not botA.goal_grid:
            botA.goal_grid = botB.goal_grid
