...
# === Print all prims for debugging camera path ===
print("\n=== Prim Paths in USD Stage ===")
for prim in stage.Traverse():
    print(str(prim.GetPath()))

# === Camera prim path (update if needed) ===
# Double check your stage output for the actual path!
...
try:
    ...
    print(f"[ERROR] Could not initialize camera at path {camera_prim_path}: {e}")
    simulation_app.close()
    exit(1)

for _ in range(200): my_world.step(render=True)
# === Step sim until camera returns valid image ===
rgb = None
for i in range(60):
...
Image.fromarray(rgb_img).save("jetbot_simple_room.png")
print("✅ Saved jetbot_simple_room.png")
